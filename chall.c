...
// ===================== Config ===================== //
#define TOL_MIN   4              // tolerance in minutes
#define TIMEOUT_S 60
#define FLAG_STR  "ptm{3z_byp4ss_ju5t_r0t4t3_th3_34rth}"
...
// CRC32 (polinomio 0xEDB88320), versione compatta senza tabella
...
// === stato condiviso tra gate e pwn_room ===
static double g_lat, g_lon;
static size_t g_dynsz; // dimensione derivata dal "conversion mismatch"
...
static void pwn_room(double lat, double lon, size_t user_size){
    ...
    // La dimensione del buffer deriva dalla LATITUDE "estesa" (conversion mismatch)
    size_t sz = user_size ? user_size : 128;
    if (sz > (1u<<20)) sz = (1u<<20); // micro-sanity per evitare crash immediati
    ...
    // leggo deliberatamente più di quanto allocato per creare l'overflow controllato
    read(0, msg, sz * 4);
...
static int solar_gate(void){
    ...
    // conversion mismatch: parse la lat vera + eventuale "size hint" come secondo numero
    long long size_hint = 0;
    if (sscanf(buf, "%lf %lld", &lat, &size_hint) < 1){
        puts("\nInvalid latitude. skill issue");
        return 0;
    }
    if (size_hint <= 0) size_hint = 128;            // default
    if ((unsigned long long)size_hint > (1ull<<20)) // hard cap 1 MiB
        size_hint = (1<<20);
